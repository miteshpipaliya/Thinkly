...
const COLLEGES = [
  ...
  "ADIT, Anand",
  "Government Polytechnic, Ahmedabad",import { useState } from "react";
import { useApp } from "../context/AppContext";
...
export default function Setup() {
  ...
  function submit() {
    if (!form.goal || !form.branch) { setErr("Please fill all required fields."); return; }
    completeSetup(form);
  }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <div style={s.sub}>Tell us about your DDCET goals so we can personalize your preparation experience.</div>

        {err && <div style={s.err}>{err}</div>}

        <div style={s.wrap}>
          <div style={s.label}>DDCET Target Marks / Rank Goal *</div>
          <input style={s.input} placeholder="e.g. 300+ marks or Top 500 rank" value={form.goal}
        ...
        <button style={s.btn} onClick={submit}>Save & Start Preparing →</button>
      </div>
    </div>
  );
}
...
const s = {
  ...
  g1: { position:"absolute", top:"-20%", right:"-10%", width:500, height:500, borderRadius:"50%", background:"radial-gradient(circle,rgba(79,142,247,0.1) 0%,transparent 70%)", pointerEvents:"none" },
  g2: { position:"absolute", bottom:"-20%", left:"-10%", width:500, height:500, borderRadius:"50%", background:"radial-gradient(circle,rgba(168,85,247,0.08) 0%,transparent 70%)", pointerEvents:"none" },
  card: { background:"rgba(255,255,255,0.035)", backdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:"40px 44px", width:"100%", maxWidth:500, position:"relative", zIndex:1 },
  step: { display:"flex", alignItems:"center", gap:8, marginBottom:28 },
  stepNum: (a) => ({ width:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, background:a?"linear-gradient(135deg,#4f8ef7,#a855f7)":"rgba(255,255,255,0.06)", color:a?"#fff":"#555" }),
  stepLine: { flex:1, height:1, background:"rgba(255,255,255,0.06)" },
  h1: { fontSize:22, fontWeight:800, color:"#e2e2f0", marginBottom:6 },
...
export default function Setup() {
  const { completeSetup, user } = useApp();
  const [form, setForm] = useState({ goal:"", branch:"", college1:"", college2:"" });
  const [err, setErr]   = useState("");

  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  function submit() {
    if (!form.goal || !form.branch || !form.college1) { setErr("Please fill all required fields."); return; }
    completeSetup(form);
  }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <div style={s.wrap}>
          <div style={s.label}>College Preference 1 *</div>
          <select style={s.select} value={form.college1} onChange={e=>set("college1",e.target.value)}>
        ...
        <div style={s.wrap}>
          <div style={s.label}>College Preference 2 (Optional)</div>
          <select style={s.select} value={form.college2} onChange={e=>set("college2",e.target.value)}>
            <option value="">Select college…</option>
            {COLLEGES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
