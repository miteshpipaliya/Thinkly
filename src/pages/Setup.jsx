/**
 * Setup.jsx — Ranklify v10
 * Removed rank goal — only branch + city + target marks
 */
...
export default function Setup() {
  ...
  const [target, setTarget] = useState(user?.profile?.goal || "");
  ...
  function submit(){
    ...
    const goal = target || "150";
    const acc  = Math.min(100, Math.round(+goal/200*1000)/10);
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#050508",display:"flex",alignItems:"center",justifyContent:"center",padding:24,position:"relative",overflow:"hidden" }}>
      ...
      <div style={{ background:"rgba(255,255,255,0.035)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:22,padding:"40px 44px",width:"100%",maxWidth:460,position:"relative",zIndex:1 }}>
        ...
        <div style={{ fontSize:21,fontWeight:900,color:"#e2e2f0",marginBottom:4 }}>One last step, {user?.name?.split(" ")[0]}! 👋</div>
        <div style={{ color:"#555",fontSize:13,marginBottom:24,lineHeight:1.7 }}>Tell us about yourself to personalize your experience.</div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16 }}>
          <div>
            <label style={lbl}>Target Marks</label>
            <input style={inp} type="number" min="0" max="200" placeholder="e.g. 150" value={target} onChange={e=>setTarget(e.target.value)} onFocus={fo} onBlur={bl}/>
          </div>
          <div>
