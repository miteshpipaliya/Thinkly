/**
 * AppContext.jsx — Ranklify v10
 * Added: readyPlayers tracking in group sessions
 */
...
const K = {
  ...
  users   : "rkl9_users",
  requests: "rkl9_requests",
  dms     : "rkl9_dms",
  gchat   : "rkl9_gchat",
  groups  : "rkl9_groups",
  invites : "rkl9_invites",
...
export function AppProvider({ children }) {
  ...
  function createGroup(seed){
    const g={
      id:Date.now(), host:user.id, members:[user.id],
      status:"lobby", results:{}, readyPlayers:[],
      seed:seed||Date.now(), createdAt:Date.now()
    };
  ...
  function acceptGroupInvite(inviteId){
    ...
    const ng=gGroups().map(g=>g.id===inv.groupId&&!g.members.includes(user.id)
      ?{...g,members:[...g.members,user.id]}:g);
  ...
  /* Mark current user as READY in a group */
  function markReady(groupId){
    const ng=gGroups().map(g=>{
      if(g.id!==groupId) return g;
      const already=(g.readyPlayers||[]).includes(user.id);
      if(already) return g;
      return{...g,readyPlayers:[...(g.readyPlayers||[]),user.id]};
    });
    LS.s(K.groups,ng); setGroups(ng);
  }

  /* Host starts — only if ALL members ready */
  function startGroupSession(groupId){
    const ng=gGroups().map(g=>g.id===groupId
      ?{...g,status:"countdown",startedAt:Date.now()+4000}:g);
    LS.s(K.groups,ng); setGroups(ng);
    /* after 4s (3-2-1-GO) flip to running */
    setTimeout(()=>{
      const ng2=gGroups().map(g=>g.id===groupId&&g.status==="countdown"
        ?{...g,status:"running"}:g);
      LS.s(K.groups,ng2); setGroups(ng2);
    },4000);
  }
  ...
  function submitGroupResult(groupId,result){
    const ng=gGroups().map(g=>{
      ...
      const allDone=g.members.every(id=>nr[id]!==undefined);
      return{...g,results:nr,status:allDone?"done":g.status};
    });
    LS.s(K.groups,ng); setGroups(ng);
  }
